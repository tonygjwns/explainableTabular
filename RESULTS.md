...
## 34. 출처 백필 판독 (2026-08-03, 커밋 ac2e2d3) — RESULTS §31이 남긴 공백 2건 닫힘

판독 규칙은 `scripts/run_provenance_backfill.py` 헤더에 실행 전 커밋돼 있었다: **재측정값이 인용
수치와 다르면 원고가 재측정값을 채택하되, 수준과 size term은 반드시 같은 런에서 온 것을 쓴다.**

**J1 (부록 B.3 회귀 size-term 행).** 재측정이 기존 배터리 아티팩트
(`synth_battery_PASS_mapenv_dd82e0a`)와 **정확히 일치**한다: recent −0.13324 → recent_old
−0.12209. size term은 +0.01114이고 같은 런의 `staleness_harm`(−0.01114)과 부호만 반대로 정확히
같다 — 이전에 없던 내부 일관성이다. 윈도우 단위로는 n=25, **25/25 전부 양수**, SD 0.0040,
평균이 floor의 **56%**.
→ 원고가 인용하던 −0.1343 / −0.1222 / +0.01212 / 61%는 **다른 하니스(`exp-reg/reg_controls`)**
의 값이었고 그 런은 수준을 저장하지 않았다(그래서 공백이었다). 사전 규칙대로 **같은 런의 3종으로
교체**: −0.1332 → −0.1221, +0.01114, **56%**, SD 0.0040. §8 한계 1의 "61%"도 같이 56%로.
**주장은 살아남는다** — 회귀에서 size term이 판정 floor의 절반을 넘고 전 윈도우에서 양수다.

**J2 (부록 C 학습가능성 게이트 행).** 단일 점 대신 꼬리 두께 스윕이 게이트를 건너는 것을 보인다:

| 팔 | learn | 판정 | 주입 회복 |
|---|---|---|---|
| 셀 기하 (학습가능 컨트롤) | 0.951 | LEARNABLE | **+0.201** |
| junk t(df=4) | 0.944 | LEARNABLE | +0.186 |
| junk t(df=2) | 0.839 | LEARNABLE | +0.150 |
| **junk t(df=1)** | **0.482** | **VACUOUS** | **+0.012** (미회복) |
| junk t(df=0.6) | 0.477 | VACUOUS | −0.002 |

→ 원고의 0.506 / 0.964 / +0.195(출처 = `AUDIT_FINAL` §C1 마크다운)를 **0.482 / 0.951 / +0.201**로
교체하고, 표현을 "단일 점"에서 **"학습가능성이 게이트 0.65를 건너는 지점에서 인증서가 공허로
바뀐다"**로 바꿨다. 목표 수치에 맞춰 junk 구성을 튜닝하지 않았다(첫 구성이 0.858 = 학습가능으로
나왔고, 그때 튜닝 대신 스윕으로 설계를 바꿨다 — REVIEW_LEDGER §4.5).

**감사 결과**: `PAPER_DRAFT_V5.md`·`_KO.md` 각 405개 수치 **unmatched 0**(직전 2), EN/KO 수치 집합
동일. 이로써 원고의 모든 계기 판독이 커밋된 아티팩트로 대조된다.
