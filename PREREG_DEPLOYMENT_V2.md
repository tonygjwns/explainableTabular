...
## 20. 선택 카드 ① 판독: 고전 탐지기 배터리 (2026-08-03 서버 실행) — `run_classic_detectors.py` 헤더 예측 적용

> ⚠ **아티팩트 대기**: 아래 수치는 서버 콘솔 출력에서 읽었다. `results/phase1/classic_detectors/
> classic_detectors.json`이 push되면 수치 감사(`audit_paper_numbers.py`)로 대조한 뒤 원고에 넣는다.
> 그전까지 이 절은 판독이지 인용 가능한 출처가 아니다.

발화율(감지/1k 샘플, 3시드 평균). DDM은 0/1 오차 스트림을 요구하므로 binclass에만 적용된다.

| 셀 | 진실 | ADWIN | DDM | KSWIN | PageHinkley |
|---|---|---|---|---|---|
| concept | 규칙 이동 | 0.167 | 0.278 | 0.000 | 0.194 |
| nuisance_proxy | 규칙 이동 | 0.111 | 0.139 | 0.028 | 0.195 |
| **concept_noise** | **규칙 이동** | 0.083 | **0.000** | 0.000 | 0.083 |
...
- **P1(노이즈 셀이 규칙 셀과 3× 이내로 발화, 60%) = 성립, 단 태스크 내 비교에서만.** 회귀 팔에서
  노이즈 셀의 ADWIN(0.472·0.417)은 규칙 셀 reg_concept(0.083)보다 **오히려 높고**, KSWIN
  (1.361·1.528 vs 1.556)과 PageHinkley(0.695·0.750 vs 0.584)는 3× 이내다. **발화는 진실을
  가리지 않는다.** (예측 문구가 태스크를 명시하지 않은 것은 설계 결함이다. 태스크를 가로지르는
  비교는 학습기가 달라 무의미하다.)
- **P2(순수 정상 셀이 양쪽보다 엄격히 낮게 발화, 75%) = 반증.** `reg_stable`은 순수 null인데
  KSWIN 1.333·PH 0.583으로 규칙 셀·노이즈 셀과 같은 수준이고, `reg_late_noisy`(고정 규칙)의
  PageHinkley 3.444는 **패널 최대**다. 탐지기는 노이즈 감쇠와 규칙 변화를 못 가르는 데 그치지 않고
  **아무 일도 없는 셀과도 못 가른다**.
- **P3(DDM은 단측이라 노이즈 셀에서 침묵, 55%) = 측정 불가.** DDM은 binclass에만 돌았는데 배터리의
  노이즈-감쇠 셀 둘은 **모두 회귀**다. 이 배터리로는 DDM의 단측성을 시험할 수 없다. 시험하려면
  이진 노이즈-감쇠 셀이 필요하며, 그것은 배터리에 없다(사후 추가하지 않는다).

### 20.2 예측하지 않았던 것 (관측으로만 기록)

1. **발화율이 진실이 아니라 태스크로 갈린다.** 회귀 셀 전부가 KSWIN 1.28–1.56·PH 0.58–3.44를
   내고 이진 셀 전부가 낮다. 진실 라벨은 이 순서를 전혀 설명하지 못한다.
2. **이진 팔에서 두 방향 실패가 동시에 보인다.** `concept_noise`(진짜 규칙 이동)가 `stable`과
   **정확히 같은** 발화(0.083/0.000/0.000/0.083)를 내고 — 미탐 — `covariate`(고정 규칙)는
   DDM 0.139·PH 0.278로 발화한다 — 오탐. 같은 배터리에서 같은 탐지기가 규칙 변화를 놓치고
   공변량 이동에 발화한다.

### 20.3 교락 명시 (이 결과를 어디까지 쓸 수 있나)

회귀 팔의 오차 스트림에는 증분 LinearRegression의 **학습 곡선 전이**가 섞여 있다. 회귀 전반의 높은
발화가 drift가 아니라 그 전이일 수 있으므로:

- **쓸 수 있는 주장**: "발화는 진실을 가리지 않는다"(회귀 팔 + 이진 팔 모두 지지).
- **쓰면 안 되는 주장**: "탐지기가 노이즈 감쇠에 *특이적으로* 발화한다"(회귀 팔 단독으로는 전이와
  구별 불가).
- **가장 깨끗한 증거**: 이진 팔의 미탐/오탐 쌍(§20.2-2). 학습기가 같고 셀만 다르다.

### 20.4 원고 반영 방향

§2의 "손실-스트림 검출기는 구성상 유형-맹목" 문장은 **논증에서 실측으로** 승격하되, §20.3의 한계와
함께 쓴다. 표는 부록으로. **§3.4는 건드리지 않는다** — 그것은 유형을 *귀속하는* 프레임에 대한
결과이고, 이것은 애초에 유형 판정을 내지 않는 탐지기에 대한 결과다. 두 문장은 서로 다른 주장이며
합치면 둘 다 약해진다.
