...
calls for retraining on recent data and moving covariates under a fixed rule do not. That noise can make a drift detector
false-alarm is known; we show the failure carries into attribution, where the repair is chosen, and
that its driver is the noise trend rather than its level. Hold the rule provably fixed, let only label noise decay over time, and the comparison
...
\textbf{Two more ways the naive instrument lies.} The certificate that should license a null --- a
window-separability AUC --- saturates to 1.000 under duplicates and entity cohorts with no covariate
shift at all, so eight of ten real datasets read as unmeasurable for a reason that is memorisation
rather than geometry. And the concept/covariate separation is hypothesis-class-relative: a kNN probe
reads pure covariate shift as concept drift ($+0.098$). Both are repaired in
Section~\ref{sec:certificates} and mapped in Section~\ref{sec:classrel}.
...
\textbf{And its blind spots, measured.} A null is worth its blind spots, so we map ours
(Section~\ref{sec:blind}) along four axes: probe class, separability, rule family, and metric. The
last is settled from outside. Against the ACA Medicaid expansion --- implemented by Pennsylvania on
2015-01-01, never adopted by Texas, and visible in the raw positive rates --- the probe read both
states null, in a fully identifiable regime and at the tightest detectable bound in the paper. The
mechanism was registered before the run: our binary score is rank-based, and an eligibility
threshold moves mass across a decision boundary without reordering anyone.
...
window size. That label noise degrades drift
detection is established, and detectors are built for it: \citet{wang2022noise} filter the false
drifts noise induces, on the premise that real streams are contaminated while detectors assume they
are not. Our claim is placed relative to that work on three axes. It concerns drift-\emph{type}
attribution rather than detection, which is the layer at which a repair is chosen; its driver is the
noise \emph{trend} rather than its level, which the growing-noise mirror establishes by flipping the
sign; and the mechanism is the inefficiency of pooled empirical risk minimisation rather than a
detector's threshold. Under the field-standard definition of real drift as any change in
$P(y \mid x)$ \citep{gama2014survey,webb2016characterizing}, noise drift \emph{is} drift, which is precisely why an
instrument claiming to detect \emph{exploitable rule change} must separate the two; we separate them
by construction and validate the separation adversarially. We are not aware of prior work that uses
the add-old-data contrast as a drift-\emph{type} identifier with verdict semantics.
...
mislead. That comparison has a false-positive channel. Its counterpart one layer down is known:
label noise makes error-rate detectors false-alarm, and detectors exist that filter those
alarms \citep{wang2022noise}. What has not been examined is whether the failure survives into
\emph{attribution}, where the answer selects a repair, and what drives it there.
...
\emph{Reading aid.} One real cell exercises most of the vocabulary (Table~\ref{tab:terms}, appendix). On
...
Rows $(x_i, y_i, t_i)$ arrive over time; the auditor sees the full labeled history (retrospective audit, not online prediction). Fix $K$ windows by timestamp-value rank (ties never straddle boundaries; per-seed boundary jitter). For each future window $W_j$ (back half), with old anchor $W_0$ (earliest window with $\ge 200$ rows and a trainable label set) and recent window $W_{j-1}$, draw size-matched samples ($N = \min(|\text{recent}|, |\text{old}|, 6000)$) and train three models of the deployed class: recent-only, old-only, recent$\,\cup\,$old. Report per-seed means over future windows of \begin{itemize} \item \textbf{decay} $=$ score(old model, held-out $W_0$) $-$ score(old model, $W_j$)---aging, mechanism-blind (documented at scale by \citealp{vela2022temporal}); \item \textbf{recency gain} $=$ score(recent) $-$ score(old) on $W_j$---adaptation value, conflates coverage and rule change; \emph{its sign is itself diagnostic (\S\ref{sec:anchors}): negative recency is impossible under one-way drift and fingerprints recurring regimes}; \item \textbf{raw staleness} $=$ score(recent) $-$ score(recent$\,\cup\,$old) on $W_j$---the attribution probe; \item \textbf{denoised staleness} $=$ score(recent) $-$ score(recent $\cup$ $(X_{\text{old}}, \hat{g}_{\text{old}}(X_{\text{old}}))$) on $W_j$, where $\hat{g}_{\text{old}}$ is a 2-fold cross-fitted model \emph{within} $W_0$ producing strictly out-of-fold hard pseudo-labels. \end{itemize}
...
\paragraph{Learnability-gated injection.} On any cell the cascade calls unidentifiable, we
plant a rule of reference strength in that dataset's own covariate geometry, check that it is
learnable in-window (held-out AUC $\ge$ 0.65, $R^2 \ge$ 0.20, or accuracy $\ge$ majority $+$ 0.10),
and only then read whether staleness recovers it. Recovery certifies that the geometry had power
and the real null is informative; persistent non-recovery of a learnable rule certifies earned
blindness; an unlearnable rule certifies nothing, and the cell is marked injection-vacuous rather
than blind. Appendix~\ref{app:instrument} gives the procedure and the identification boundary it
operationalises.
...
The three rows that carry the paper's argument are read in Section~\ref{sec:channel}; the rest are
controls whose truth is fixed by construction, and the instrument must return the right verdict on
all fourteen before it touches real data. Two readings appear nowhere else: a third noise channel,
correlated with the rule-carrying feature, fires the raw arm at +0.026 and is defused by the
denoised arm at +0.006; and the denoiser survives a small old window (600 rows, folds of 300) with
no false positive (+0.004) and no false negative (+0.429). Appendix~\ref{app:instrument} covers floor
...
tools; the fourth is settled from outside, against a rule change documented in law. It comes
before the application because a verdict of no rule change is worth only its blind spots.
...
\emph{membership}, in the same geometry, at the same strength. The detection literature reports
the same blind spot one layer down: standard detectors miss drift confined to a small
subpopulation \citep{giobergia2024localized}. Our contribution here is not the blind spot but its
measurement inside a certificate, where it determines what a null is worth.
...
Under proper scores the same windows turn positive on the treated state and stay far
smaller on the control, which localises the blindness to the score and points at a repair without
licensing one: that arm carries no certificate, the control moves too, and the floor it clears is
calibrated in AUC units (Appendix~\ref{app:properscore}).
...
With the instrument certified and its blind spots mapped, we spend it on the question that
motivated building it: does industrial tabular data contain rule change a deployed model could
exploit? The answer is a map, and it is worth what Section~\ref{sec:blind} says it is worth. Every
number below is scoped to a tree-ensemble probe class, to the separability at which its
certificate was earned, to the rule families that certificate was tested against, and to a
rank-based score. We state the qualifiers with the result rather than after it.
...
\textbf{What the instrument can see, and where the map does not move.} A map of nulls is only as
informative as the sensitivity of the instrument that drew it. Over 23 synthetic streams and all
seven INSECTS variants, monotone and single-switch rule changes fire 9/9, and recurring regimes are
correctly silent with a negative-recency fingerprint that is impossible under one-way drift; on the
audited panel that flag is silent everywhere (Appendix~\ref{sec:anchors}). Re-running all eight cells
across the held-out deployment gap leaves 8/8 verdicts identical and no industrial cell firing
CONCEPT, so the aggregate is not re-scoped to train segments (Appendix~\ref{sec:gap}).
...
drift-type attribution at all. We do not claim to have discovered that noise fools drift
machinery: for detection that is established, with detectors built to resist it. We claim the
failure survives into type attribution, that a trend drives it, and that it was large enough to
have fooled us. And
...
\textbf{On process.} This project's ledger records nine positive findings that dissolved under
scrutiny before the present result; the ninth is Section~\ref{sec:diagnosis}, and it is the only one
the measuring instrument itself diagnosed. Appendix~\ref{app:instrument} records what that discipline
cost and caught, and the future work it leaves open.
...
The instrument is a single scikit-learn script; every stochastic step is seeded and every output
artifact embeds its commit hash, argv, library versions and UTC timestamp. The synthetic battery is
byte-reproducible on a fixed environment, and any change to the instrument must re-pass it
bit-identically before a new reading is admitted (Section~\ref{sec:regate}). The pre-registration
freezes thresholds, cascade, seed protocol and aggregate reading, with results appended as
read-only sections and per-experiment predictions committed before execution. Appendix~\ref{app:repro} gives the compute, the environment, the data-access route, and two disclosures we owe the
reader: one deviation from the registered procedure, and one decision constant we know to be wrong
in a direction we did not act on.
...
\section{Instrument details}\label{app:instrument}
...
wrong interpreter (Appendix~\ref{app:repro}). We release the audit trail as part of the artifact, dissolutions included.
...
\paragraph{Scores, intervals and window construction.}

Scores: AUC (binary), accuracy (multiclass), $-$RMSE on z-scored targets (regression). CIs are Student-$t$ over 10 seeds; each seed re-draws a 90\% row subsample, re-jitters window boundaries, and re-samples all training sets. These are seed-level intervals over heavily overlapping subsamples and are anti-conservative for tiny effects (\S\ref{sec:anchors} calibrates the resulting no-evidence band on no-drift anchors); verdict-level inference does not rest on them alone---the confirmatory fresh-seed replication (\S\ref{sec:panel}) is the operative stability check, and per-seed values are emitted with every run so that alternative constructions (window-block bootstrap, split-half) can be applied without re-execution. The pre-registered estimand is \emph{exploitable mean-rule drift}: a change in the decision-relevant functional of $P(y \mid x)$ that makes old labels contradict the current rule for the deployed hypothesis class. This is deliberately narrower than the field-standard ``any change in $P(y \mid x)$'' \citep{morenotorres2012unifying,webb2016characterizing}---the narrowing is the point, because the wider definition classifies label-noise drift as concept and thereby licenses retraining that cannot help.
...
\section{The blind-spot map in full}\label{app:properscore}

\paragraph{Class relativity: the full matrix.}
...
\paragraph{The within-cell ladder, and why it could not be built.}
...
\paragraph{Family relativity across geometries.}
...
\paragraph{The proper-score follow-up.}
...
\section{Across the deployment gap}\label{sec:gap}
...
\textbf{\texttt{reg\_early\_noisy}} & \textbf{rule fixed, noise decays} & \textbf{0.472} & --- & 1.361 & 0.695 \\
\textbf{\texttt{reg\_xdep\_noise}} & \textbf{rule fixed, noise decays} & \textbf{0.417} & --- & 1.528 & 0.750 \\
stable & rule fixed & 0.083 & 0.000 & 0.000 & 0.083 \\
\textbf{covariate} & \textbf{rule fixed} & 0.111 & \textbf{0.139} & 0.000 & \textbf{0.278} \\
\textbf{\texttt{reg\_stable}} & \textbf{rule fixed (pure null)} & 0.083 & --- & \textbf{1.333} & \textbf{0.583} \\
