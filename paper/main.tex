...
instrument. We study \emph{staleness harm}, a deployment-native probe (does adding old examples to a
...
instrument---a cross-fitted \emph{denoised staleness} arm with a noise gate and a mapped abstention
...
replication (10/10 verdicts stable) then yields an identifiability \emph{map}, not a detection table:
...
repaired protocol; our propositions are scoping lemmas, not theory contributions.
...
Table~\ref{tab:map} gives the map. The pre-registered aggregate reading:
...
12\}$ (at $K = 10$ reproducing the v2 headline \textbf{bit-for-bit}, $+0.023900573591226625$---the raw
...
the gate correctly co-flagging its noise component), \texttt{sine\_reoccur2} $+0.047$. INSECTS:
