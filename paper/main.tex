...
the deployed tree-ensemble class (0/8 audited---and 0/5 among the cells whose identifiability is
certified; the other two are uninformative rather than negative)};
...
\textbf{0/8 audited---0/5 among the certified cells}, the remaining two being uninformative rather
...
\item \emph{A pre-registered identifiability map of industrial tabular ML} (\S\ref{sec:map}): 0/8 exploitable
   mean-rule drift over the audited panel---0/5 once the two certificate-less cells are separated
