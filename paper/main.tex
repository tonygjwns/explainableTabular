...
\textbf{no industrial dataset shows exploitable mean-rule drift above its detectable floor for
the deployed tree-ensemble class (0/8 audited---and 0/4 among the cells whose identifiability is
certified; the other four are uninformative rather than negative)};
the sole robust positive is designed drift; the one prior industrial positive is \emph{diagnosed} by
the instrument itself as label-noise decay; every remaining cell is either an identifiable null
or carries a certificate verdict---verified, blind, refused, or (one cell) unstable. Anchor
streams fix the sensitivity profile: monotone and
...
\textbf{0/8 audited---0/4 among the certified cells}, the remaining four being uninformative rather
than negative; the designed-drift stream is the sole positive (and its denoised staleness \emph{exceeds} the
...
minted it; two industrial datasets earn a verified no-concept certificate via injection recovery,
while \emph{all four} of the industrial ``unidentifiable'' cells end without an earned
certificate---three because their injections are unlearnable (a fact the
naive protocol would have laundered into ``earned blindness'') and the fourth because its
learnability score straddles the gate across seed sets; the panel's only earned blindness is
Electricity, which is not one of the industrial eight. Anchor streams (\S\ref{sec:anchors}) give the
...
\item \emph{A pre-registered identifiability map of industrial tabular ML} (\S\ref{sec:map}): 0/8 exploitable
   mean-rule drift over the audited panel---0/4 once the four certificate-less cells are separated
   out, which is the map's point rather than a caveat to it;
...
attribution at all. Finally, the map does not adjudicate what a \emph{deep} time-aware
architecture could exploit: it fixes what the dominant deployed class has to exploit, and the
battery (\S\ref{sec:classmatrix}) is the entry bar any richer probe must pass before its
verdicts join the map.
...
``unidentifiable'' industrial cells end without an earned certificate---three fail the learnability
gate outright, and the fourth (\texttt{homesite\_insurance}) scores within 0.05 of the gate and
flips across seed sets. The certificate is explicitly
...
which is why we carry the 0/4 certified count alongside the pre-registered 0/8
...
\subsection{Results: 10/10 confirmatory-stable; industrial mean-rule drift 0/8 audited, 0/4 certified}\label{sec:results}
...
\texttt{ecom\_offers} & UNIDENTIFIABLE & $-0.001$ & $-0.007$ & 1.25 & 1.000 & --- & \emph{vacuous}---injection unlearnable \\
\texttt{homecredit\_default} & UNIDENTIFIABLE & $-0.007$ & $+0.004$ & 1.29 & 1.000 & --- & \emph{vacuous} (80 proxy features stripped) \\
weather & UNIDENTIFIABLE & $-0.012$ & $-0.011$ & 0.93 & 0.994 & --- & \emph{vacuous} \\
\texttt{homesite\_insurance} & UNIDENTIFIABLE-INERT & $-0.004$ & $-0.002$ & 1.14 & 1.000 & $-0.052$ & \emph{unstable} (vacuous$\leftrightarrow$earned across seed sets) \\
\bottomrule
\end{tabular}

\vspace{2pt}
{\scriptsize $D$ $=$ \texttt{D\_strip}, the group-aware separability median ($D^\star = 0.96$ routes
to the injection control). Rec.\ $=$ recovered staleness of the planted reference rule (2.5\,rad);
``---'' $=$ the injection was unlearnable, so no recovery number is admissible (vacuity discipline),
and n/a $=$ the cell never routes to the injection control. Recovery values move by ${<}0.01$
between seed sets except on \texttt{homesite\_insurance}, whose certificate is unstable for exactly
that reason.}
\end{table}
...
\textbf{0/4}, because \texttt{ecom\_offers}, \texttt{homecredit\_default}, weather (certificate
refused---the planted probe rule is unlearnable in their geometry) and
\texttt{homesite\_insurance} (certificate unstable across seed sets) contribute no evidence in
either direction. We report both numbers wherever the aggregate appears: the first is what was
...
the old rule cleanly once label noise is removed, the signature of genuine rule change. \emph{All
four} industrial blindness claims that a naive protocol would have granted end without an earned
certificate---three because the planted probe rule is unlearnable in those geometries (their
top-variance features cannot carry it), the fourth because its learnability score straddles the
gate---a distinction invisible without the learnability gate. The strongest cells in the map are,
...
floor co-occurring with the noise decay---no sub-floor claim is made in either direction.) We are
...
from industrial tabular ML: \textbf{four of the eight cells are certificate-less} (three refused
for unlearnable injections, one unstable across seed sets), and \textbf{none of the eight is
...
much larger $N$ is invisible here, so every $\delta$ bound and the 0/8 (0/4 certified) aggregate are statements \emph{at probe
...
interaction-borne, or subpopulation-local rule change is not certified, so ``verified
no-concept'' should be read as verified against that family. An injection-family sweep is the
corresponding hardening step.
...
\section{N-scaling and cross-lens agreement (post-hoc robustness checks)}\label{app:rebuttal}
...
\midrule
decision floor & 0.02 & CONCEPT magnitude bar, shared across AUC / accuracy / z-scored $-$RMSE & battery concept magnitudes (planted effects 10--27$\times$ the floor); per-metric rescaling moves no cell (\S\ref{sec:battery}) & sub-floor CI-positives $=$ no-evidence band, calibrated on no-drift anchors (2/5 land at 1/10--1/20 of the floor) \\
...
injection strength & 2.5 rad & reference rotation magnitude & clears the floor on learnable geometry (recoveries $+0.16$ to $+0.55$) & single reference family (\S\ref{sec:discussion}, Lim.~6); the family sweep is the hardening step \\
