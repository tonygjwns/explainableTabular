...
is worth reporting as a warning: on ecom_offers the linear probe fires the \emph{denoised} arm alone
...
probe fixed --- delivery_eta 0.521 $\to$ 0.540 $\to$ 0.555 $\to$ 0.801, homecredit_default 0.721 $\to$ 0.796 $\to$
...
cell (delivery_eta $-0.002$ $\to$ $-0.013$; homecredit_default +0.013 [+0.010, +0.016] at k = 5, where
...
sberbank_housing & \textbf{NOISE-DRIFT-CONFOUNDED} & +0.024 / +0.033 (fires) & \textbf{$-0.015$ / $-0.011$} & \textbf{2.11 / 2.21 (fires)} & 1.000 & --- & diagnosed (Section~\ref{sec:diagnosis}) \\
cooking_time & INJECTION-RECOVERED & $-0.011$ & $-0.018$ & 0.92 & 0.966 & \textbf{+0.546} & \textbf{verified no-concept} \\
delivery_eta & INJECTION-RECOVERED & $-0.012$ & $-0.014$ & 0.89 & 0.999 & \textbf{+0.332} & \textbf{verified no-concept} \\
...
ecom_offers & UNIDENTIFIABLE & $-0.001$ & $-0.007$ & 1.25 & 1.000 & --- & \emph{vacuous} --- injection unlearnable (0/4 families) \\
homecredit_default & UNIDENTIFIABLE & $-0.007$ & +0.004 & 1.29 & 1.000 & --- & \emph{vacuous} (0/4 families; 80 proxy features stripped) \\
weather & INJECTION-RECOVERED $\dagger$ & $-0.012$ & $-0.011$ & 0.93 & 0.994 & \textbf{+0.183} & \textbf{verified no-concept} (2/3 learnable families) \\
homesite_insurance & UNIDENTIFIABLE-INERT & $-0.004$ & $-0.002$ & 1.14 & 1.000 & $-0.001$ & \textbf{blindness earned} (2/2 learnable families, stable) \\
\bottomrule
\end{tabular}
\end{table}
...
segment. cooking_time's separability \emph{falls} below D\* (0.927), against our expectation that a
...
\textbf{Monotone and single-switch rule changes fire: 9/9.} River: agrawal_abrupt +0.045,
...
\textbf{reg_early_noisy} & \textbf{rule fixed, noise decays} & \textbf{0.472} & --- & 1.361 & 0.695 \\
\textbf{reg_xdep_noise} & \textbf{rule fixed, noise decays} & \textbf{0.417} & --- & 1.528 & 0.750 \\
...
reg_late_noisy & rule fixed & 0.528 & --- & 1.278 & \textbf{3.444} \\
\bottomrule
\end{tabular}
\end{table}
