...
## ★★★ 현행 (2026-08-03) — 실험 큐 4일치 실행 완료, **열린 결정은 하나뿐**. 새 탭은 여기부터

**한 줄**: `[C] full-span` · `[D1] 주입 패밀리×캐리어 스윕` · `ACS 자연 실험` · `day-4(E1~E4)`까지
실행·기록·원고 반영이 끝났다. **남은 결정은 계기 우선 재구성 여부 하나**이고, 나머지 작업은 전부
거기에 달려 있다.
...
| | |
|---|---|
| 원고 | **v4.9** — EN/KO/tex 3파일 동기. 부록 B.4·B.5·B.6, 한계 6 재작성, **한계 7 신설**, 부록 C floor 비판 |
| 사전등록 | `PREREG_DEPLOYMENT_V2.md` §16(설계 보정)·§17·§18, `PREREG_ACS_EXTENSION_2026-07-31.md` §10·§11 |
| 헤드라인 | **0/8 감사 · 0/5 인증** (인증서 없는 셀 4 → 2) |
| 서버 | `run_day4.sh` 실행 중 — 결과 대기 |
| **열린 결정** | **[PAPER_V5_SKELETON.md](PAPER_V5_SKELETON.md) 채택 여부** |
...
1. **수치 감사** — `scripts/audit_paper_numbers.py`(신규). 원고의 수치 주장을 아티팩트와 대조.
   이미 이 방식으로 "three of four" 오류를 한 번 잡았다.
2. **V5 재구성 골격 검토** → 채택 시 §3·§6 신설 초안.
3. LaTeX 재컴파일 — 로컬 툴체인 없음. **급하지 않음**, 제출 직전 1회.
