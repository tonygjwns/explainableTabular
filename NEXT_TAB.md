...
**한 줄**: 계기-우선 재구성 **채택 → day-4 판독 → V5 §1~§9 폴드 → 통독 1회 → KO 미러**까지 끝났다.
정본 = [PAPER_DRAFT_V5.md](PAPER_DRAFT_V5.md)(EN) · [PAPER_DRAFT_V5_KO.md](PAPER_DRAFT_V5_KO.md)(KO),
두 파일의 수치 집합 동일·dangling 참조 0. **남은 것은 tex 이식 + A9(태그/OSF) + 읽기 전용 통독**이고,
**열린 항목의 정본 목록은 [REVIEW_LEDGER.md](REVIEW_LEDGER.md) §4다** — 이 파일의 이력 블록에 있는
"다음 = …" 문장들은 전부 V5 이전이라 그대로 믿으면 안 된다.
