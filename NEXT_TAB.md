...
**한 줄**: 열린 결정이던 계기-우선 재구성을 **채택**했다. day-4(E1~E4)가 돌아와 판독·기록·반영까지
끝났고, V5의 **§3(실패 채널)·§6(계기의 맹점)·§7(적용 = 강등된 지도)** 초안이
[PAPER_DRAFT_V5_SECTIONS.md](PAPER_DRAFT_V5_SECTIONS.md)에 있다(수치 204개 전부 아티팩트 대조 통과).
**다음 = §1 초록·서론 재작성**(주어가 "산업 데이터에 drift 없다" → "drift 검출이 노이즈에 속는다"로
바뀌는 최대 작업), 그다음 §4·§5(V4 §3·§4 이전)·§8·§9, KO 미러와 tex는 EN이 굳은 뒤 한 번에.

### 지금 상태 (커밋 `bacf455`)

| | |
|---|---|
| 원고 | **V4 = v4.9 그대로 살아 있음**(EN/KO/tex 동기). **V5는 별도 파일**에 §3·§6·§7 초안 — 버리려면 그 파일만 삭제 |
