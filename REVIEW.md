...
## 9. 외부 피드백 & 수정된 입장 (2026-06-05)

신규 멤버/검토자 피드백을 받았고, **대체로 타당하여 §5의 결론을 하향 조정**한다.

**가로지르는 모순 (핵심)**: 최종 판정이 *성능(부차 청구)* 위에서, *부서진 메커니즘*으로,
*좁은 증거* 위에서 내려졌는데 **메인 청구(해석가능성)는 한 번도 측정되지 않았다.**
→ "프로젝트 실패"는 과잉. **메인 청구는 미결(undecided).**

**과잉 사망선고였던 것 (철회/보류)**:
- *표현적 redundancy의 일반화*: in-distribution 예측에만 적용. 외삽·해석엔 미적용.
- *"외삽에 해로움"(Test 9)*: **Fourier 기저는 t>1에서 주기적으로 회귀(환각)** — 추세 외삽
  불가. 외삽 음성이 메커니즘이 아니라 *기저 선택*의 산물일 수 있음. λ·기저 ablation 없었음.
- *"retrieval 무용"*: 검색이 K=1000→1개로 **붕괴**했는데 anti-collapse 장치(엔트로피/
  load-balance/top-k)가 없었음 → **부서진 retrieval을 테스트**한 셈.
- *"실데이터에 concept 없음"*: concept 지표(ρ(t,y), 8슬라이스 centroid)가 둔하고 누수
  오염(+36%). P(y|x) 변화를 직접 재지 못함.
- *해석 청구*: 학습 V_k가 프로토타입을 비해석적으로 만들고(설계가 메인청구 배반),
  실험에서 한 번도 측정 안 함.

**방법론 결함 (수용)**: 검정력 거꾸로(null 유력 Test1=10시드 / 정보성 큰 Test9=단일·불안정),
결정적 비교 p=.11을 "견고"로 격상, 단일 벤치 일반화, 테스트 누수, §5에서 동일 결과의
견고·취약 동시 등재, 사전등록-수행 불일치.

**단단하게 남는 것**: 구현 정확(합성 +87%), z-지름길 진단, *정적 in-distribution*에서
시간-피처를 못 이김(단 underpowered → 강한 증거 아님).

### 9.1 버리기 전 재실험 계획 (피드백 우선순위)
1. **검색 붕괴 수정** — 엔트로피/load-balance 정규화 또는 sparse top-k.
2. **외삽 가능한 시간 기저** — Fourier(주기) → trend/저차 다항/단조 상태 (`--time-basis` ablate).
3. **외삽 재실험** — temporal split × 2~3 데이터셋 × 다중 시드 + (λ, 기저) ablation.
   → 외삽 음성이 *메커니즘 vs 기저* 어느 탓인지 분리 (이게 §6 방향을 실제로 결정).
4. **메인 청구(해석) 측정** — 합성(진짜 w(t) 알려짐)에서 *궤적이 실제 드리프트를 복원하는가*를
   falsifiable하게.
5. **방법론 수정** — 결정적 비교에 시드 집중, 누수 차단 후 concept 재측정,
   concept를 *P(y|x) 변화 직접*(시점별 모델 교차적용 손실↑)으로.

→ 위 (1)(2)(3)이 끝나기 전에는 "메커니즘이 외삽/실데이터에 무용"이라 단정하지 않는다.

### 9.2 재피드백 반영 — 정밀화 + 계획 v2 (2026-06-05)

**정밀화 수정**
- redundancy는 in-dist null을 **허용(permit)할 뿐 증명(prove)하지 않음**. 닫는 것은
  *표현적* 경로뿐 → 관측된 동률 = 표현적 우위 부재 **+ 다른-경로(샘플효율/최적화) 이득의
  경험적 부재**(underpowered). "redundancy로 설명"이라 over-claim 금지.
- **§4/§5의 "concept 없음"은 쪼갠다**: **강한 covariate drift(시점분류기 AUC≈1.0)는 단단**
  (정당한 측정). **concept는 미확정**(ρ(t,y)는 둔한 지표 — 여기만 보류).
- **z-지름길은 *as-built* 속성**(붕괴 검색 + 학습 V_k)이지 아키텍처 클래스의 영구 속성이 아님.
  계획 1·2·V_k 교체가 그 구성을 해체하므로 mem_gap이 달라질 수 있음.

**계획 v2 (보강·재배열)**
- **(0, 신규 핵심) V_k = 실제 라벨/인스턴스(비파라메트릭, §6라)** — 붕괴 수정과 결합 필수.
  학습 V_k면 검색을 퍼뜨려도 나를 정보가 없어 "퍼졌지만 무용"한 새 아티팩트만 생김.
- **(1) anti-collapse**(엔트로피/load-balance/top-k) — (0)과 동시에.
- **(2) 외삽 가능 기저**(trend/poly/상태) 토글, 기저는 메커니즘의 일부로 *분리* ablate.
- **(5→먼저) covariate-조정 concept 측정**으로 외삽 데이터셋을 **선별**(exploit 가능한 것만);
  importance weighting/매칭으로 covariate 초과손실 제거(안 하면 +36% 오염 재현).
- **(3) 요인설계**: {구조: memory vs time-feature} × {기저: Fourier vs trend} × λ,
  **baseline의 t 인코딩을 같은 기저로 정합** → 구조/기저/상호작용 분리 귀속.
- **(4) 해석=충실성 게이트**: 복원 지표 **사전 정의**(궤적 vs 진짜 w(t) 정렬 + 우연 기준선);
  합성 복원은 *필요조건*일 뿐 실데이터 payoff 아님(실패→해석청구 즉사 / 통과→유용성 별도).
- **사전 결정규칙**을 각 실험에 명시(사후 재litigation 방지).

**실행 순서(싸고 결정적 우선)**: 먼저 **(4) 합성 해석복원 + (0+1+2) 재설계 메커니즘 +
작은 (3) 외삽 1~2개(5로 선별)** 로 go/no-go. 둘 다 실패 → 너그러운 해석에서도 아이디어
사망 → **§6(다) 현상-분석**으로 전환(강한 covariate는 이미 단단). 성공 → 확장.
전제: 완전 성공해도 산출물은 좁으니(외삽·일부 데이터셋·합성 충실성) (다)와 기대값을 미리 견줄 것.
