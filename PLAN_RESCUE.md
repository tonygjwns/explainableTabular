...
- **구조 기여를 깨끗이 가르는 법(round 6)**:
  (a) ★**같은-모델 시간-조건 on/off ablation** — 시간-TabR에서 drift=0(프로토타입/유사도 시간-불변)로
  고정. 같은 파라미터·아키텍처, 시간만 토글 → 아키텍처 confound 0 (재설계 메커니즘으로 *측정가능-concept*
  데이터에서 다시 한 올바른 Test 1). (b) **파라미터 수 정합**(time-TabR이 MLP+t보다 크면 승리가 용량 탓 배제).
  (c) **이득 국소화** — 구조 이득이 concept 큰 영역(late-overlap)에 집중되는지 F3 concept 위치에 매핑(기전 증거).
- **Q2a 주의**: 현재 `run_elec2 --time-basis trend` 는 *학습-프로토타입* 메모리 → §4.2 정보없음 결함 잔존
  → null은 무정보(구조 실패로 해석 금지). 인스턴스 V_k 빌드 전 *값싼 진단*일 뿐.
- **Q1 robustness(우선순위↑)**: 헤드라인의 "충실한 메커니즘조차 못 돕는다" 논거가 Q1에 의존 →
  큰-회전(≥180°/다주기, 기저 정합) 1회로 동적범위 넓혀 충실성 PASS를 robust화(현 floor 0.894).
