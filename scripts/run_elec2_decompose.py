...
def decompose(name, X, y, t_raw, task, seeds, strides=(1, 5, 25)):
    order = np.argsort(t_raw, kind="stable")          # stream order
    Xs, ys = X[order], np.asarray(y)[order]
    out = {"dataset": name, "task": task, "n": int(len(ys))}
    ...
    if task == "binclass":
        ...
        g_no = _gap_seeds(Xs[em], ys[em], Xs[lm], ys[lm], task, seeds)      # baseline (no lag)
        g_lag = _gap_seeds(Xlag[em], ys[em], Xlag[lm], ys[lm], task, seeds)  # with lag feature
...
def main():
    ...
    for name, data in jobs:
        ...
        r = decompose(name, X, data.train.y, data.train.t_raw, data.task, seeds)
        results.append(r)
        print(f"\n  [{name}] task={r['task']}")
        print("   (1) THINNING (short-range autocorrelation removed as stride grows):")
        for c in r["thinning"]:
            gt = (f"{c['gap_mean']:+.3f}[{c['gap_ci'][0]:+.3f},{c['gap_ci'][1]:+.3f}]"
                  if c["gap_mean"] is not None else "   abstain")
            print(f"     stride={c['stride']:>2d} n={c['n_kept']:>6d}  gap={gt}  [{c['verdict']}]")
        lg = r["lagged_label"]
        if lg:
            print(f"   (2) LAGGED-LABEL: y_t-1→y AUC={lg['autocorr_auc_y_from_ylag']:.3f} "
                  f"(autocorr strength); gap no-lag {lg['gap_no_lag']:+.3f} -> with-lag "
                  f"{lg['gap_with_lag']:+.3f} (shrink {lg['shrink']:+.3f}) [{lg['verdict_with_lag']}]")
        bn = r["bayes_noise_proxy"]
        print(f"   (3) BAYES-NOISE proxy: achievable acc early {bn['achievable_acc_early']:.3f} "
              f"-> late {bn['achievable_acc_late']:.3f} (drift {bn['achievable_acc_drift']:+.3f})")
